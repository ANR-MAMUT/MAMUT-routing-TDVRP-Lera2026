# Lera2026 TDVRP benchmark family (TD-IGP-Gehring&Homberger, 200–1000 customers)

Satellite benchmark repository of [MAMUT-routing](https://github.com/ANR-MAMUT/MAMUT-routing) (ANR MAMUT, ANR-22-CE22-0016), mounted there as `benchmarks/TDVRP/Lera2026`. Self-contained: instances and best-known solutions ship together; the time-dependent travel times are stored as a compact IGP specification (`igp-profile` model) and materialize deterministically on load — no committed arrival-time-function sidecars (a complete n=1000 ATF sidecar would weigh ~60–90 MB gzipped).

This is the **time-window-free twin** of [`MAMUT-routing-TDVRPTW-Lera2026`](https://github.com/ANR-MAMUT/MAMUT-routing-TDVRPTW-Lera2026): identical instances with the `time_windows` array dropped (capacity, fleet, service times, horizon and all td content — including both sha256 values — are byte-identical to the TDVRPTW sibling). The horizon is the TDVRPTW-repaired one, so the twins stay exactly comparable. The time-window repair section below describes where that horizon comes from.
